...
import com.aidev.four.files.ProjectManager
import com.aidev.four.files.ShizukuInstaller
...
internal class ProjectManagerState(activity: Activity) {
    ...
    fun startBuild(projectDir: File) {
        ...
        buildJob = scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val readyFile = File(_activity.filesDir, "home/ubuntu-rootfs/.aidev-rootfs-ready")
                if (!readyFile.isFile) {
                ...
                val rootfs = File(_activity.filesDir, "home/ubuntu-rootfs").absolutePath
    ...
    fun workspaceDir(activity: Activity): File = File(activity.filesDir, "home/ubuntu-rootfs/root/Workspace")

    private fun workspaceDir(): File = File(_activity.filesDir, "home/ubuntu-rootfs/root/Workspace")
    ...
    private fun createProotProcess(innerCmd: String): Process {
        val nativeDir = _activity.applicationInfo.nativeLibraryDir
        val rootfs = File(_activity.filesDir, "home/ubuntu-rootfs").absolutePath
        val proot = File(nativeDir, "libproot.so").absolutePath
        val prootLoader = File(nativeDir, "libproot_loader.so").absolutePath
        val aidevHome = File(_activity.filesDir, "home").absolutePath
        val prootLibDir = File(_activity.filesDir, "home/proot-lib").absolutePath
        val prootTmpDir = File(_activity.cacheDir, "proot_tmp").apply { mkdirs() }.absolutePath
        ...
        val shellCmd = buildString {
            ...
            append("ANDROID_SDK_ROOT=/host-home/android-sdk ")
            append("GRADLE_USER_HOME=/host-home/gradle-cache ")
            ...
            val escaped = innerCmd
                .replace("\\", "\\\\")
                .replace("\$", "\\\$")
                .replace("\"", "\\\"")
            append("/bin/sh -c \"$escaped\"")
        }
        ...
        pb.environment()["LD_LIBRARY_PATH"] = "$prootLibDir:$nativeDir"
