...
#   -o, --output DIR       输出父目录（默认 ~/projects）
#   -f, --force            覆盖已存在的项目目录
#   -h, --help             显示帮助
...
if ! echo "$JAVA_VERSION" | grep -qi '17\.'; then
    echo "  警告: 当前 JDK 不是 17 (检测到: ${JAVA_VERSION})"
    echo "         构建可能不兼容"
fi
...
echo "═══════════════════════════════════════════"
echo "  目录: ${PROJECT_DIR}"
echo "  构建: cd ${PROJECT_DIR} && ./gradlew assembleDebug"
echo ""
echo "  编译运行后即可看到: \"Hello, ${APP_NAME}!\""
echo "═══════════════════════════════════════════"
