...
### 待续（④⑤⑥⑦，①②③ 合入后继续）
- **④ 最小 CI（GitHub Actions）**：`compileDebugKotlin` + `testDebugUnitTest` + shell 套件（`bash app/src/test/sh/run.sh`），作为回归闸门。无 `.github/workflows`。
- **⑤ 治理漂移修正**：`AGENTS.md` 的「黄金组合」版本号（AGP 8.7.0 / Gradle 8.9 / Kotlin 2.0.20 / BOM 2024.06.00）已过期，实际为 AGP 9.0.1 / Kotlin 2.0.21 / compileSdk·targetSdk 36 / BOM 2024.12.01；同步之。另 `docs/architecture.md` 实为 `opencode-architecture.md` 符号链接（OpenCode API 参考），补一份真正的 App 架构文档或修正指针。
- **⑥ `SafeCommandGuard` 收紧**：当前为大小写敏感子串黑名单，可被 `rm  -rf /`（双空格）、`dd of=`、命令替换/`sh -c "$(...)"` 绕过，且 `REQUIRE_CONFIRM` 被静默降级为 BLOCK。改为解析命令模型 + 白名单，至少封死 `eval`/`sh -c` 包裹。
- **⑦ 大文件拆分**：`BuildBridgeService`(1166)、`UbuntuBootstrapScripts`(834 内嵌 shell)、`TerminalShellAssets`(641) 把构建流水线/JDK-aapt2 安装脚本以 Kotlin 字符串内嵌，无法 lint、diff 痛苦。把内嵌 shell 外置到 `assets/scripts`、构建流水线拆 coordinator + steps。
