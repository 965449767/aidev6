# 非安卓开发指南（宇宙A / PRoot 侧能力边界）

> 适用：在 AIDev 终端（宇宙A，PRoot Ubuntu 24.04 aarch64）内做安卓以外的开发。
> 核心约束：编译隔离——安卓 APK 由宇宙B 编译；宇宙A 侧只做编辑/提交，**原生交叉编译受限**。
...
| Rust | rustc/cargo 1.97 | ✅ 可用 | 已装 4 个 Android target；**但 NDK 链接器为 x86_64 预编译，A 侧无法跑**（见 §三），仅 host target 可编 |
...
以下因架构分工或环境限制，**宇宙A 侧不可用或结果不可验证**，请勿在 A 侧误用：

1. **aapt2 完整解析**：宇宙A 本地无 `/Android` 的 aapt2，`aidev-apk-info` 自动降级为 unzip+strings 基础模式（仅提取包名等少量信息）。完整解析（权限列表/minSdk/targetSdk 等）需在宿主侧或宇宙B。
2. **NDK 原生交叉编译**：NDK（含 clang/cmake）为 linux-x86_64 预编译，rootfs 是 aarch64 且缺 x86_64 加载器 → `clang`/`cmake` 无法执行。原生模块（JNI/Rust-Android）请在宇宙B 编译或交给宿主。
3. **anr / 部分 crash 抓取**：受设备安全策略约束（如 `cat /data/anr/*` 被拒），原始命令可能拿不到。以宿主侧 `aidev-doctor` / 日志桥转发为准。
4. **deploy 安装结果**：安装本身经宿主 Shizuku 真装成功（弹窗为 Shizuku 授权，确认即装）；A 侧 `pm list packages` 软校验常因设备不可见拿不到输出，故 `aidev-deploy` 现已**以 `aidev-install` 退出码为安装真相**，软校验失败仅告警不致命。

## 四、推荐工作流

- Python/Node/Rust(host)/单文件 C：直接在 A 侧编辑+运行。
- 安卓 APK：A 侧 `aidev-create-android-project` 生成 → `aidev-build-request` 提交宇宙B → 产物回传 A 侧。
- 原生/JNI/Rust-Android：交宇宙B 或在宿主侧用 aarch64 NDK 编译。
- 安装自测：A 侧 `aidev-deploy --apk <apk> --pkg <pkg>`，结果以 JSON 回传（installed 反映宿主 Shizuku 真实结果）。
