...
> ⚠️ **已退役（2026-07-17）**：本文件描述的「自我进化闭环」已在「人类驱动重构」中整体移除。
> 宿主不再自动改码 / 自动重建 / 自动修复；终端在无 AI Agent 时也必须完整可用。
> 仅保留供**人类排查**的崩溃落盘：`CrashGuard` 把宿主崩溃写本地文件，`aidev-error-why` 等命令读取；
> 构建失败仅把完整日志落到 `logs/<project>/last-build-failure.log`，不再写 `self-evolution/*` 回流文件。
> 本文件留作历史参考，请勿据此实现自动能力。
